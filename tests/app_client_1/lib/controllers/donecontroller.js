angular.module("salesHubApp").controller("DoneController", function ($scope, $filter, $timeout, Warehouse, localstorage, $sce, $rootScope) {
    ...
    //load page
    $scope.load = function () {
        var promise = Warehouse.CallApi('/orgnization/inbox/done?org=' + $scope.UserOrganization._id, 'GET', false);
    ...
    $scope.itemDone = function (_Inbox) {
        _Inbox.Activities.Status = 'Active';
    ...
    $scope.multiDone = function () {
        for (var i = 0 ; i < $scope.allInboxList.length; i++) {
            if ($scope.allInboxList[i].Check == true) {
                $scope.allInboxList[i].Activities.Status = 'Active';
