angular.module("salesHubApp").controller("InboxController", function ($scope, $filter, $timeout, Warehouse, localstorage, $sce, $rootScope) {
    ...
    $scope.allUsers = [];
    $scope.CalendarLink = window.location.protocol + "//" + window.location.host + "/api/calendar/" + userObject._id;
    //load page
    $scope.load = function () {

        $("#snoozeInbox").daterangepicker({
            "opens": "right",
        ...
        function cb(start, rangeName) {
            ...
            for (var i = 0 ; i < $scope.allInboxList.length; i++) {
                if ($scope.allInboxList[i].Check == true) {
                    $scope.allInboxList[i].Activities.Due = start.format('MM-DD-YYYY');
        ...
        var promise = Warehouse.CallApi('/orgnization/inbox?org=' + $scope.UserOrganization._id, 'GET', false);
        promise.error(function (data) {
            console.log(data);
            if (data.readyState) {
                console.log('connection error');
    ...
    $scope.showInboxDetails = function (oneInbox,index) {
        $("#snoozeOneInbox" + index).daterangepicker({
            "opens": "right",
            "singleDatePicker": true,
            "ranges": {
                '1 month': [moment().add(1, 'months')],
        ...
        function cb(start, rangeName) {
            console.log(start.format('MM-DD-YYYY'));
            oneInbox.Activities.Due = start.format('MM-DD-YYYY');
            var promise = Warehouse.CallApi('/lead/inbox', 'put', oneInbox, false);
            promise.error(function (data) {
                console.log(data);
                if (data.readyState) {
                    console.log('connection error');
